...
int
hex_to_bytes(const char *hex, uint8_t *out, size_t out_cap)
{
	if (!hex || !out) {
		return -1;
	}
	...
	for (size_t i = 0; i < byte_len; ++i) {
		int hi = hex_digit_value((uint8_t) hex[2 * i]);
		int lo = hex_digit_value((uint8_t) hex[2 * i + 1]);
		if (hi < 0 || lo < 0) {
			return -1;
		}
...
void
bytes_to_hex(const uint8_t *bytes, size_t len, char *out_hex)
{
...
brute_force_single_byte_xor(const char *hex_input,
    uint8_t *out_plain, size_t *out_len, uint8_t *out_key, double *out_score)
{
